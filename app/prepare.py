...
modeModel = str(input())
if modeModel != 'reconstruct_model' and modeModel != 'train_model':
    print('Error: Incorrect mode. Available: "train_model", "reconstruct_model"', file=sys.stderr)
    
if modeModel == 'train_model':
    print('Mode "train_model" is not available now, use mode "reconstruct_model"')
params.whatUWant = modeModel


def prepare_reconstruction(datasetFolder, model, imgNamePattern, poseNamePattern, initialPtsNamePattern, outputFolder, N_viewPairs4inference, resol, BB, viewList):
    """
