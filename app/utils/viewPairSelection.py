...
def __argmaxN_viewPairs__(viewPairs, w_viewPairs, N_argmax):
    ...
    argmaxN_viewPairs = np.repeat(viewPairs[None, ...], N_validCubes,
                                  axis=0)[indice_cube, indice_N_max]  # (N_viewPairs, 2) --> (N_validCubes, N_viewPairs, 2) --> (N_validCubes, N_argmax, 2)
    argmaxN_w = w_viewPairs[indice_cube, indice_N_max]  # (N_validCubes, N_argmax)
    return (argmaxN_viewPairs, argmaxN_w)


def viewPairSelection(
    cameraTs_np,
    e_viewPairs,
    d_viewPairs,
    validCubes,
    ...
    theta_viewPairs = \
        camera.viewPairAngles_wrt_pts(cameraTs=cameraTs_np,
            pts_xyz=cubeCenters_xyz[validCubes])[..., None]  # (N_validCubes, N_viewPairs, 1)
    d_viewPairs = d_viewPairs[validCubes][..., None]  # (N_validCubes, N_viewPairs, 1)
    w_viewPairs = np.empty((N_validCubes, N_viewPairs),
                           dtype=np.float32)

    # TODO: change fn to accept pair-by-pair inputs rather than the cube-by-cube inputs

    for _batch in utils.yield_batch_npBool(N_all=N_validCubes,
            batch_size=int(math.floor(float(batchSize) / N_viewPairs))):
        N_batch = _batch.sum()
        ...
        _e_viewPairs = e_viewPairs[validCubes][_batch][:,
                viewPairs.flatten()].reshape((N_batch, N_viewPairs, 2
                * D_embedding))
        N_features = 2 * D_embedding + 2

        # (N_batch, N_viewPairs, N_features),

        features_viewPairs = np.concatenate([_e_viewPairs,
                d_viewPairs[_batch], theta_viewPairs[_batch]],
                axis=-1).astype(np.float32).reshape((N_batch
                * N_viewPairs, N_features))

        # TODO: check

        w_viewPairs[_batch] = \
            viewPair_relativeImpt_fn(features_viewPairs,
                n_samples_perGroup=N_viewPairs)  # (N_batch*N_viewPairs, N_features) --> (N_batch, N_viewPairs)

    # select N_argmax viewPairs

    (selected_viewPairs, selected_similNet_weight) = \
        __argmaxN_viewPairs__(viewPairs=viewPairs,
                              w_viewPairs=w_viewPairs,
                              N_argmax=N_viewPairs4inference)
