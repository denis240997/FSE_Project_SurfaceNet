...
def readImages(
    ...
    for (i, viewIndx) in enumerate(viewList):
        ...
        imgPath = os.path.join(datasetFolder, imgNamePattern.replace('#'
                               , '{:03}'.format(viewIndx)).replace('@',
                               '{}'.format(viewIndx)))
...
def cropImgPatches(
    ...
    range_hw_max = np.maximum(range_h[:, 1] - range_h[:, 0], range_w[:,
                              1] - range_w[:, 0])  # elementwise maximum. (N_patches,). The longest projection on the axes. If small, upsample the image in order to crop a fixed size patch.

    # determine how many layers of the image pyramid

    patchSizeRatio = float(patchSize) / range_hw_max  # (N_patches,)
    logRatio = (np.log(patchSizeRatio)
                / np.log(pyramidRate) if pyramidRate
                is not 1 else np.ones(patchSizeRatio.shape))  # element-wise log, if pyramidRate == 1, there are infinite pyramid layers. In this case we only set one img in the pyramid.
    logRatio_int = np.floor(logRatio)  # -1.2 --> -2; 3.7 --> 3

    patches = np.empty((N_patches, patchSize, patchSize, img_c),
                       dtype=img_dtype)
    ...
    for _logRatio_int in set(logRatio_int):
        ...
        _imgResize = scipy.ndimage.interpolation.zoom(input=img,
                zoom=(_resizeRate, _resizeRate, 1.), output=img_dtype,
                order=interp_order)  # (img_h*_resizeRate, img_w*_resizeRate, 3/1)
        ...
        _patch_h_min = (center_h[_select] * _resizeRate).astype(np.int) \
            - patchSize_r  # (N_logRatioInt, )
        _patch_w_min = (center_w[_select] * _resizeRate).astype(np.int) \
            - patchSize_r
        ...
        _pixel_h_int = _patch_h_min[:, None, None] \
            + _patchRelativeCoords[0:1]  # (N_logRatioInt, 1, 1) + (1, patchSize, patchSize)
        _pixel_h = np.clip(_pixel_h_int, a_min=0, a_max=_imgResize_h
                           - 1)  # (N_logRatioInt, patchSize, patchSize), clip to range [0, _imgResize_h) for indexing
        _pixel_w_int = _patch_w_min[:, None, None] \
            + _patchRelativeCoords[1:2]
        _pixel_w = np.clip(_pixel_w_int, a_min=0, a_max=_imgResize_w
                           - 1)
...
def img_hw_cubesCorner_inScopeCheck(hw_shape, img_h_cubesCorner,
                                    img_w_cubesCorner):
    """
    ...
    inScope = (range_h_min >= 0) & (range_h_max <= img_h) \
        & (range_w_min >= 0) & (range_w_max <= img_w)  # (N_cubes,) inScope check and select perticular _logRatio_int.
    return inScope
