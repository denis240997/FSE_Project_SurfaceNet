...
def initializeCubes(
    ...
    N_along_axis = lambda _min, _max, _resol: int(math.ceil((_max
            - _min) / _resol))
    N_along_xyz = [N_along_axis(BB[_axis][0] - safeMargin, BB[_axis][1]
                   + safeMargin, cube_stride_mm) for _axis in range(3)]  # how many cubes along each axis
...
def quantizePts2Cubes(
    ...
    if BB is not None:
        ...
        inBB = np.array([np.logical_and(pts_xyz[:, _axis] >= BB[_axis,
                        0] - safeMargin, pts_xyz[:, _axis] <= BB[_axis,
                        1] + safeMargin) for _axis in
                        range(3)]).all(axis=0)
    ...
    cubes_ijk_unique = \
        np.unique(cubes_ijk_1d).view(cubes_ijk_floor.dtype).reshape((-1,
            3))  # (N_cubes, 3)
...
def readPointCloud_xyz(pointCloudFile='xx/xx.ply'):
    ...
    pcd_xyz = np.c_[pcd['vertex']['x'], pcd['vertex']['y'], pcd['vertex'
                    ]['z']]
    return pcd_xyz
